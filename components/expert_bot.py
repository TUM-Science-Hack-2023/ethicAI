...
class ExpertBot():
    def __init__(self, expertise : str) -> None:
        assert expertise in prompts.keys(), "Provided expertise is not recognized! Please check available ones in components/expert_bot.py"
        self.expertise = expertise

        self.task_title = prompts[self.expertise]["title"]
        self.task = prompts[self.expertise]["definition"]
        self.scoring_scheme = prompts[self.expertise]["criteria"]

        self.prompt = None
        self.summary_prompt = None

    def get_completion(self, use_case, model="gpt-3.5-turbo"):
        """
